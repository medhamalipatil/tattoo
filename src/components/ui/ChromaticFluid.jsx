...
void main() {
  ...
  // Let's use the theme background colors for the fluid base!
  vec3 baseColor = vec3(30.0/255.0, 32.0/255.0, 54.0/255.0); // Pine-Emerald Background
  color += baseColor * 0.3;
...
const ChromaticFluid = ({
  ...
  return (
    <div
      ref={hostRef}
      className={`chromatic-container ${className}`}
      style={{ height, ...style }}
      {...props}
    >
      <canvas
        ref={canvasRef}
        className="chromatic-canvas"
      />
      <div className="chromatic-content">{children}</div>
    </div>
  );
};
